...
app.post('/api/duel-bot', (req, res) => {
    ...
    if (my > bot) {
        ...
        saveDB(db);
        return res.json({ ok: true, my, bot, result: 'win', stars: user.stars, commission });
    } else if (my < bot) {
        user.stars -= b;
        saveDB(db);
...
app.post('/api/duel-accept', (req, res) => {
    ...
    const my = Math.floor(Math.random() * 11) + 2;
    const his = Math.floor(Math.random() * 11) + 2;
    let result, winner, loser;
    if (my > his) { winner = opponent; loser = challenger; result = 'win'; }
    else if (my < his) { winner = challenger; loser = opponent; result = 'lose'; }
    else { result = 'draw'; }
    let commission = 0;
    if (winner) {
    ...
    delete db.duels[duelId];
    saveDB(db);
    res.json({ ok: true, my, his, result, stars: user.stars, commission });
});
...
app.post('/api/withdraw', (req, res) => {
    ...
    saveDB(db);
    res.json({ ok: true, stars: user.stars, message: 'Заявка создана. Напишите @referator' });
});
...
app.post('/api/admin/stats', (req, res) => {
    ...
    res.json({
        ...
        activeDuels: Object.keys(db.duels).length,
...
app.post('/api/admin/users', (req, res) => {
    if (req.body.password !== ADMIN_PASSWORD) return res.json({ ok: false, error: 'Нет доступа' });
    const db = loadDB();
    res.json({ ok: true, users: Object.values(db.users).map(u => ({ username: u.username, stars: u.stars, banned: u.banned, created: u.created })) });
});
...
app.post('/api/admin/give', (req, res) => {
    ...
    const amt = parseInt(amount);
    if (isNaN(amt)) return res.json({ ok: false, error: 'Неверная сумма' });
    u.stars += amt;
    saveDB(db);
